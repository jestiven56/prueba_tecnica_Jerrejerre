...
function validarFormato($linea) {
    ...
    // Verificar que tenga al menos los campos requeridos (email y código)
    if (count($datos) < 4) {
        return false;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (count($registros_validos) > 0) {
            ...
            $stmt = $pdo->prepare("INSERT INTO usuarios (email, nombre, apellido, codigo) VALUES (?, ?, ?, ?) 
                                  ON DUPLICATE KEY UPDATE nombre = VALUES(nombre), apellido = VALUES(apellido), codigo = VALUES(codigo)");
